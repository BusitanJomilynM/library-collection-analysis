...
<form action="{{ route('restoreUpdate', $book->id) }}" method="POST">
    @csrf
    @method('GET')
    ...
    <div class="form-group">
        ...
        <input class="form-control @error('book_edition') is-invalid @enderror" type="text" name="status" id="status" value="0" hidden> 
        @error('book_edition')
            <span class="text-danger">{{$message}}</span>
        @enderror
    </div>
